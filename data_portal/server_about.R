adata = function(env_serv) with (env_serv, {
  ...
  biodef <- reactive({
    data <- filter(definitions,Term == input$biome_def)
    def <- data$Definition
  })
  bioex <- reactive({
    data <- filter(definitions,Term == input$biome_def)
  ...
  output$bio_def2 <- renderText({
    paste(as.character(biodef()))
  })
  
  output$bio_def3 <- renderText({
    paste(as.character(bioex()))
  })
  ...
  video2 <- ("https://www.youtube.com/embed/_goJyTgj8gA")
